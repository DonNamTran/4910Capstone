...
<?php
    ...
    if($sponsor === "All Sponsors") {
        ...
        if($al_type === "Point Changes") {
            $points_query = mysqli_query($connection, "SELECT * FROM point_history JOIN drivers ON point_history.point_history_driver_id=drivers.driver_id WHERE point_history_date BETWEEN '$start_range' AND '$end_range_format' ORDER BY point_history_driver_id, point_history_associated_sponsor, point_history_date;");  
        ...
        if($al_type === "Point Changes") {
            $points_query = mysqli_query($connection, "SELECT * FROM point_history JOIN drivers ON point_history.point_history_driver_id=drivers.driver_id JOIN organizations ON organizations.organization_username=drivers.driver_associated_sponsor WHERE organization_username='$sponsor' AND point_history_date BETWEEN '$start_range' AND '$end_range_format' ORDER BY point_history_driver_id, point_history_associated_sponsor, point_history_date;");  
            $row_count = $points_query->num_rows;
            $counter = 0;
